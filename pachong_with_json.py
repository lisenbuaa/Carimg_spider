...
class ImageSpider(object):
    ...
    def getimage(self,url,word,id):
        ...
        # 创建目录，用于保存图片
        directory = './dst/{}/'.format(id)
        # 如果目录不存在则创建新目录
        if not os.path.exists(directory):
            os.makedirs(directory)
        
        for img_link in img_link_list:
            filename = '{}{}_{:04}.jpg'.format(directory, id, self.i)
...
if __name__ == '__main__':
    ...
    # 打开并读取JSON文件
    count = 0
    with open('./source/need_improved_cls.json', 'r', encoding='utf-8') as file:
        data = json.load(file)

    # 遍历列表中的每个字典，提取"name_from_new"和"id"
    for i, item in enumerate(data):
        if i >= count:
            print("已经爬取到第",str(count),"车系了")
            name_from_new = item.get('name_from_new', '未知')  # 如果不存在，则返回'未知'
            id_value = item.get('id', '未知')  # 如果不存在，则返回'未知'
            if name_from_new == '未知' and id_value == '未知':
                continue
            spiderer = ImageSpider()
            spiderer.run(name_from_new, id_value)
            count = count + 1
